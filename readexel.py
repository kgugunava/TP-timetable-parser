...
table = pd.read_excel('downloaded_file.xlsx', skiprows=10, header=None, keep_default_na=False)
    ...
    for index, row in table.iterrows():
        ...
        if row[x] != '' and row[y] != '' and row[1] != 'Дни':
            ...
            for i in range(len(s)):
                lesson += [re.split(' - | (?=[А-Я][^(/.)])', s[i])]
            # с помощью регулярных выражений разбиваем строчки с расписанием на нужные категории
            print(lesson)
            spitted_lesson = SplittingLessons(lesson)
            name = spitted_lesson.get_name_of_lesson()
            type_of_lesson = spitted_lesson.get_type_of_lesson()
            print(name, type_of_lesson)
    print('\n\n\n')
